...
import { ChevronLeft, ChevronRight, FileUp, ScanLine, X } from "lucide-react";
import type { OctCanvasFrame } from "../../components/oct/OctCanvas";
...
type VolumeState = TiffState | SingleState | null;
...
export function ProjectAnnotatePage() {
  ...
  useEffect(() => {
    if (!volume || volume.kind !== "tiff") return;
    ...
    (async () => {
      ...
      try {
        ...
        if (name.endsWith(".tif") || name.endsWith(".tiff")) {
          const worker = new Worker(new URL("../../workers/tiffWorker.ts", import.meta.url), {
            type: "module",
          });
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              if (name.endsWith(".tif") || name.endsWith(".tiff")) {
                ...
                setVolume(null);
                setLoadError("Unsupported file type for display (image/TIFF only for now).");
              }
            } catch (err) {
              setVolume(null);
